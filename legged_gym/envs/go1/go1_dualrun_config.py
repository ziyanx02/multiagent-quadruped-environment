...
class Go1DualrunCfgPPO(Go1FieldCfgPPO):
    ...
    class runner(Go1FieldCfgPPO.runner):
        ...
        run_name = "".join(["Skill",
        ("Multi" if len(Go1DualrunCfg.terrain.BarrierTrack_kwargs["options"]) > 1 else (Go1DualrunCfg.terrain.BarrierTrack_kwargs["options"][0] if Go1DualrunCfg.terrain.BarrierTrack_kwargs["options"] else "PlaneWalking")),
        ("_propDelay{:.2f}-{:.2f}".format(
                Go1DualrunCfg.sensor.proprioception.latency_range[0],
                Go1DualrunCfg.sensor.proprioception.latency_range[1],
           ) if Go1DualrunCfg.sensor.proprioception.delay_action_obs else ""
       ),
        ("_pPenV" + np.format_float_scientific(-Go1DualrunCfg.rewards.scales.penetrate_volume, trim= "-", exp_digits= 1) if getattr(Go1DualrunCfg.rewards.scales, "penetrate_volume", 0.) < 0. else ""),
        ("_pPenD" + np.format_float_scientific(-Go1DualrunCfg.rewards.scales.penetrate_depth, trim= "-", exp_digits= 1) if getattr(Go1DualrunCfg.rewards.scales, "penetrate_depth", 0.) < 0. else ""),
        ("_noPush" if not Go1DualrunCfg.domain_rand.push_robots else ""),
        ("_virtual" if Go1DualrunCfg.terrain.BarrierTrack_kwargs["virtual_terrain"] else ""),
        ])
        resume = False
        # resume = True
